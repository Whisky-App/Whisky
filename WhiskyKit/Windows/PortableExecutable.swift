...
public struct COFFFileHeader: Hashable {
    ...
    init(data: Data) throws {
        ...
        offset += 4
        self.machine = data.extract(UInt16.self, offset: offset) ?? 0
        offset += 2
        self.numberOfSections = data.extract(UInt16.self, offset: offset) ?? 0
        offset += 2
        self.timeDateStamp = data.extract(UInt32.self, offset: offset) ?? 0
        offset += 4
        self.pointerToSymbolTable = data.extract(UInt32.self, offset: offset) ?? 0
        offset += 4
        self.numberOfSymbols = data.extract(UInt32.self, offset: offset) ?? 0
        offset += 4
        self.sizeOfOptionalHeader = data.extract(UInt16.self, offset: offset) ?? 0
        offset += 2
        self.characteristics = data.extract(UInt16.self, offset: offset) ?? 0
        offset += 2
...
public struct PEFile: Hashable {
    public var coffFileHeader: COFFFileHeader
